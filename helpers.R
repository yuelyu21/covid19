...
Draw_world_map <- function(data_Map, geo_data){
  ...
  bins <- c(0,1000,10000,30000,80000,100000,300000,1500000,max(geo_data$TotalCases))
  bins2 <- c(0,100,500,1000,3000,10000,30000,100000,max(geo_data$TotalDeaths, na.rm = T))
...
countrymap <- function(country_name){
  ...
  country_data <- countrydata[province_order,] %>%
    dplyr::mutate(TotalCases = as.numeric(TotalCases),
  ...
  if (country_name == "China"){
    ...
    bins_country2 <- c(0,5,10,30,1000,max_death)
...
read_news <- function(country_name){
  ...
  response %>% select(title, description, url, publishedAt) %>% dplyr::filter(!is.na(title))
}
